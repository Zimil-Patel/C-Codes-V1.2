...
void main(){
	...
	char email[32], domain[10];
	
	const char constDomain[10] = "@gmail.com";
	
	printf("Create Email Id : ");
	gets(email);
	
	short emailLength = strlen(email),
			i, j = 0,
			specialCharCheck = 0,
	...
	if (emailLength > 10){
		...
		if (domainCheck == 0){
			...
			if (alphaCheck){
				...
				if (((email[0] >= 'a' && email[0] <= 'z') || (email[emailLength - 10] >= 'a' && email[emailLength - 10] <= 'z')) || 
					((email[0] >= 'A' && email[0] <= 'Z') || (email[emailLength - 10] >= 'A' && email[emailLength - 10] <= 'Z')) || 
					((email[0] >= '0' && email[0] <= '9') || (email[emailLength - 10] >= '0' && email[emailLength - 10] <= '9'))){
